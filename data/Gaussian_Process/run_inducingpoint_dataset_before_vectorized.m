...
t_train_gp = t_ip_local_gp_train;   % compatibility with old result fields
fprintf('局部GP: %.4fs\n', t_ip_local_gp_train);
...
for mi=1:numel(AllModes)
    ...
    if ismember(lower(cur),dac_methods)
        ...
        while iter<max_iter
            ...
            %判断dac是否已经收敛
            if max(abs(Zeta(:)-Zeta_prev(:)))<1e-5, break; 
            end
        end
        ...
        Xi_initial_average = mean(Pi, 2);  % [p_dim x 1 x NumInducingPoints]
        ...
        while iter<max_iter
            ...
            if max(abs(Xi(:)-Xi_prev(:)))<1e-5, break;
            end
        end
        t_ip_consensus = toc(tic_consensus);
        iter_converge=iter;
    ...
    % Timing-breakdown aliases for easier table generation
