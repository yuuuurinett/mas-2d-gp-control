...
SigmaN_set = [0.01; 0.01; 0.01; 0.01; 0.01; 0.01];
...
norm_vartheta_all_set = sqrt(sum(vartheta_all_set .^ 2));
semilogy(AxesObj_OverallError,t_set,norm_vartheta_all_set,'-');
%%
FigureObj_TriggerEvent = figure('Name','Trigger Events');
AxesObj_TriggerEvent = axes(FigureObj_TriggerEvent);
trigger_set_plot = trigger_set;
trigger_set_plot(trigger_set_plot == 0) = nan;
sum(trigger_set,2)
plot(AxesObj_TriggerEvent, ...
	t_set,trigger_set_plot + kron(ones(1,numel(t_set)),(1:AgentQuantity)') - 1,'*');
axis(AxesObj_TriggerEvent,[min(t_set),max(t_set),0.5,AgentQuantity + 0.5]);
