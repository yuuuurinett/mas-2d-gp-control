function ET_MAS_GP_Leader_OnceComparison_TestFunc(EventTriggerType, ...
...
warning off;
%% Event Trigger Type
% NoTrigger, centralized, distributed, continuous, exact
...
t_end = 40;
t_step = 0.01;
if numel(EventTriggerType) >= 10
	if strcmpi(EventTriggerType(1:10),'continuous') && numel(EventTriggerType) >= 12
		t_step = str2double(EventTriggerType(12:end));
		EventTriggerType = 'continuous';
	elseif strcmpi(EventTriggerType(1:10),'continuous')
		t_step = 0.01;
		EventTriggerType = 'continuous';
	end
end
...
DomainScale = 1;
X_min = DomainScale * [-1.5,-0.6];
X_max = DomainScale * [ 1.5, 0.6];
...
Local_X_min = DomainScale * [-1.5,-0.6; -1.5,-0.6; -1.5,-0.6; -1.5,-0.6];
Local_X_max = DomainScale * [ 1.5, 0.6;  1.5, 0.6;  1.5, 0.6;  1.5, 0.6];

% Individual Setting
MaxDataQuantity_set = 200 * ones(AgentQuantity,1);
SigmaN_set = 1 * [0.01; 0.01; 0.01; 0.01];
...
for AgentNr = 1:AgentQuantity
	...
	Sigma_update_set = nan(AgentQuantity,1);
	Sigma_update_set(AgentNr) = LocalGP_set{AgentNr}.SigmaN;
	for Bidirection_NeighbourNr = 1:numel(AgentBidirection_NeighbourSet)
		...
		Sigma_update_set(Bidirection_NeighbourAgentNr) = LocalGP_set{Bidirection_NeighbourAgentNr}.SigmaF;
	end

% 	Sigma_update_aggregation = sqrt(1 / (sum(Sigma_update_set .^ (-2)) / numel(Sigma_update_set)));
% 	Sigma_update_aggregation_set(AgentNr) = Sigma_update_set' * Sigma_update_set.^(-2) / Sigma_update_aggregation^(-2) / numel(Sigma_update_set);

% 	var_row_vector_SingleAgent = ET_MAS_GP_Leader_vector2cell(Sigma_update_set, numel(Sigma_update_set), 1);
	[~, ~,sigma_aggregated] = ET_MAS_GP_Leader_GPAggregation_SingleAgent( ...
		AgentNr, AgentBidirection_NeighbourSet, ...
		Sigma_update_set', cell(1,AgentQuantity), beta, gamma);
	Sigma_update_aggregation_set(AgentNr) = sigma_aggregated;
...
if is_RandomInitialState
	x0 = 2 * (rand(AgentQuantity * x_dim,1) - 0.5) .* kron(ones(AgentQuantity,1),X_max');
else
	x0 = [];
	for AgentNr = 1:AgentQuantity
		s_i_set = s_set_cell{AgentNr}.s_i_set;
...
for t_Nr = 1:numel(t_set) - 1
	...
	for AgentNr = 1:AgentQuantity
		...
		if strcmpi(EventTriggerType,'exact')
			f_hat_matrix(:,AgentNr) = ET_MAS_GP_Leader_UnknownDynamics(x_i,q_dim,SystemOrder);
		end
	...
	[~,x_temp_set] = ode45(MultiAgent_Dynamics_FunctionHandle, ...
		[t,t + t_step],x_all);
	%
...
%% Saving Result
if nargin >= 5
	save(SaveResultAddress);
	fprintf(NotificationText);
end
warning on;
end