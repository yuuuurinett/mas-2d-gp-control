...
for t_Nr = 1:T-1
    ...
    mu_model = cell(AgentQuantity,1);
    var_model = cell(AgentQuantity,1);
    for k = 1:AgentQuantity
        [mu_model{k},var_model{k}] = ...
            predict_gp_mean_variance(LocalGP_set{k},x_all_matrix);
    end
    ...
    for n = 1:AgentQuantity
        ...
        for k = 1:AgentQuantity
            mu_all(:,k)  = mu_model{k}(:,n);
            var_all(:,k) = var_model{k}(:,n);
        end
