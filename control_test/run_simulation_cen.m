function [TrackingError_vector, t_set] = run_simulation_cen(CurrentMode, SaveFolderName, SaveFileName)
rng(0);
%% 1. System Parameters
...
for t_Nr = 1:numel(t_set)-1
    ...
    % CEN 聚合：每个 agent n 在自己状态点上，汇集所有 agent 的局部预测后集中聚合
    for n = 1:AgentQuantity
        x_n = x_all_matrix(:, n);
...
fprintf('Mode: CEN-%s done, total=%.2fs\n', CurrentMode, toc);
