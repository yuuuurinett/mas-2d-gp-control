function [TrackingError_vector, t_set] = run_simulation_nbr(CurrentMode, SaveFolderName, SaveFileName)
rng(0);
%% 1. System Parameters
...
for n = 1:AgentQuantity
    % L(n,j) < 0 表示 j 是 n 的邻居
    nbrs = find(L(n,:) < 0);
    NeighborSet{n} = [n, nbrs];  % 包含自身
end
...
for t_Nr = 1:numel(t_set)-1
    ...
    % NBR 聚合：每个 agent n 只使用自己和直接邻居的局部 GP 预测
    for n = 1:AgentQuantity
        x_n   = x_all_matrix(:, n);
        nbrs  = NeighborSet{n};
        N_nbr = numel(nbrs);
        ...
        for d = 1:y_dim
            ...
            switch lower(CurrentMode)
                ...
                case 'bcm'
                    prec = sum(1./var_a) - (N_nbr-1)/prior_var;
                    mu_agg(d) = sum(mu_a./var_a) / max(prec, eps);
                case 'rbcm'
...
fprintf('Mode: NBR-%s done, total=%.2fs\n', CurrentMode, toc);
