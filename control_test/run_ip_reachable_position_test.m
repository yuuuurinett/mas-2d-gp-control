function Summary = run_ip_reachable_position_test(do_run)
%RUN_IP_REACHABLE_POSITION_TEST Full-domain versus known-reference domain.

if nargin < 1 || isempty(do_run), do_run = true; end
repo_root = fileparts(fileparts(mfilename('fullpath')));
addpath(genpath(repo_root));
result_folder = fullfile(repo_root,'result','ip_reachable_position_test');
...
baseline_file = fullfile(repo_root,'result', ...
    ...
    'poe_ip_dac_position100_velocity6.mat');
reachable_point_file = fullfile(result_folder, ...
    'inducing_points_position100_velocity6_formation_reachable.mat');
generate_formation_reachable_inducing_points(reachable_point_file,0.10);
...
env_values = {'20','all_agents','600',reachable_point_file, ...
    '0.5','1.5','0.000001','0','1','0.005', ...
    '0.001','0.0001','10','0.25','0.25'};
for env_i = 1:numel(env_names), setenv(env_names{env_i},env_values{env_i}); end

reachable_result = fullfile(result_folder,'poe_ip_dac_reachable_position.mat');
if do_run
    run_simulation_inducing_point('poe',result_folder, ...
        'poe_ip_dac_reachable_position',true,0.4,0.1,[],1);
end

labels = {'Full [-1.5,1.5]';'Known-reference + 10% margin'};
files = {baseline_file;reachable_result};
