...
%SimulationModes = {'distributed_POE', 'masked_POE'};
SimulationModes = {'distributed_POE'};
...
for ModeNr = 1:NumModes
    ...
    Xi_diff_POE_history = zeros(1, numel(t_set));

    if strcmp(CurrentMode, 'masked_POE')
        Kappa_P = 10;
        % 初始化诱导点和MaskedGP
        P = gp_masked_poe_init( ...
            LocalGP_set, AgentQuantity,AdjMatrix, NeighbourSet, ...
            NumInducingPoints, InducingPoints_Coordinates);
        Zeta_vector_inducing = zeros(4, AgentQuantity, NumInducingPoints);
        [MaskedGP_POE, Zeta_vector_inducing] = gp_masked_poe_update( ...
            P, Zeta_vector_inducing, AdjMatrix, NeighbourSet, ...
            Kappa_P, AgentQuantity, NumInducingPoints, 0, ...
    ...
    if strcmp(CurrentMode, 'distributed_RBCM')
    ...
    for k = 1:numel(t_set)
        ...
        switch CurrentMode
            ...
            case 'masked_POE'
                ...
                [MaskedGP_POE, Zeta_vector_inducing] = gp_masked_poe_update( ...
                    P, Zeta_vector_inducing, AdjMatrix, NeighbourSet, L, ...
        ...
        [~, x_output] = ode45(MultiAgent_Dynamics_Handle, ...
            [CurrentTime, CurrentTime + t_step], AgentState_matrix(:));
    ...
    fprintf('Mode: %s done, 总时间=%.2f s, GP=%.2f s, ODE=%.2f s\n', ...
     ...
     if strcmp(CurrentMode, 'distributed_POE')
        figure('Color','w');
        semilogy(t_set, Xi_diff_POE_history, 'r-', 'LineWidth', 1.5);
        xlabel('$t$', 'Interpreter', 'latex', 'FontSize', 12);
        ylabel('Xi difference (all agent pairs)', 'FontSize', 12);
        title('Distributed POE Xi Convergence');
        xlim([t_start, t_end]);
        grid on; box on;
    ...
    if strcmp(CurrentMode, 'masked_POE')
        figure('Color','w');
        semilogy(t_set, MaskedGP_Y_diff_history, 'g-', 'LineWidth', 1.5);
        xlabel('$t$', 'Interpreter', 'latex', 'FontSize', 12);
        ylabel('$\sum \|Y_i - Y_j\|_F$', 'Interpreter', 'latex', 'FontSize', 12);
